...
## 6) Jaehrliche Datenpflege (`annual_baseline.json`)
1. Datei `annual_baseline.json` enthaelt den offiziellen Jahresmodus fuer die Weltkarte (`Jaehrlich (Offiziell)` im Header).
2. Quellen eintragen:
   - `sourceNote` als kurzer Methodenhinweis
   - `sources` als Liste aus `{ "title", "url" }`
3. Datensatz aktualisieren:
```bash
node scripts/build_annual_baseline.js
```
Optional mit externer Rohdatei:
```bash
node scripts/build_annual_baseline.js /pfad/zur/deiner_input.json
```
4. Commit + Push, damit GitHub Pages den Jahresdatensatz ausliefert.
5. Im Frontend pruefen:
   - Header auf `Jaehrlich (Offiziell)` umschalten
   - `Quellen`-Button oeffnen und Links pruefen
   - Karte und Leaderboard zeigen Jahreswerte statt Monatsumfrage.
